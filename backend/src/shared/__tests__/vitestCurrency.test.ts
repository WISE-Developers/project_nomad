...
describe('vitest advisory floor, backend (#384)', () => {
  ...
  it('declares a floor that cannot resolve back into the vulnerable range', () => {
    ...
    expect(declared, 'backend/package.json no longer declares vitest').toBeDefined();

    const floor = (declared as string).replace(/^[^0-9]*/, '');

    expect(
      compareSemver(floor, VULNERABLE_AT_OR_BELOW),
      `backend/package.json declares vitest "${declared}", whose floor ${floor} is ` +
