...
## 2. <a name="two"></a> Goals / non-goals
...
## <a name="three"></a> 3. The one real domain change: a `Dataset` aggregate
...
## <a name="four"></a> 4. Capabilities & attachment seams
...
## <a name="five"></a> 5. Dormancy — how "off" stays off
...
## <a name="six"></a> 6. Known constraints (not hand-waved)
...
- **The standard needs governance** — its version authority must be decoupled from Nomad's release cycle, or other tools' conformance breaks (see standard [§12](#twelve)).

## <a name="seven"></a> 7. Scope
...
## <a name="eight"></a> 8. Proposed sequence

**Standard → A → B.** Ratify (or at least stabilise) the `.ond` standard draft, then build A against it, then B.

## <a name="nine"></a> 9. What we'd like from the team (review points)

1. **The `Dataset` aggregate** ([§3](#three)) — does the landscape → fire → run model fit Nomad's domain, and how should it relate to `FireModel`?
2. **The extension seams** ([§4](#four)) — are the proposed attachment points the right ones, and is the A2 export-registration change acceptable?
3. **The dormancy approach** ([§5](#five)) — does the config-gated factory branch for A5 satisfy "off stays off"?
4. **Streaming** ([§6](#six)) — preferred approach for large-raster import/export.
5. **Scope/sequence** ([§7](#seven)–[§8](#eight)) — agreement on MVP boundary and the standard-first order.

## <a name="ten"></a> 10. Open questions
