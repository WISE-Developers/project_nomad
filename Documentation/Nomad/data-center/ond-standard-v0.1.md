...
6. **Progressive.** A dataset may carry only the essentials or the full record; both are valid (see §10).
...
| **Conformant reader/writer** | A tool that fully honours this standard (see §11). |

## 4. Container

- A `.ond` file is a **ZIP archive** with the extension `.ond`.
- It MUST contain a single root **`manifest.json`** (§6).
- All other content lives in folders (§5). There are no other files at the archive root.
...
Notes:
- `inputs/dem/` and `inputs/fuels/` are **folders**, not single files, to allow **versioning over time** (§7) and derived products.
...
- Originals are retained adjacent to canonical artifacts (§8).

## 6. The manifest (`manifest.json`)
...
| `crs` | The dataset coordinate reference system (see §7). |
| `extent` | Spatial bounding box of the landscape. |
| `temporalExtent` | Earliest/latest run timestamps present. |
| `landscape` | Inventory of DEM/fuels artifacts **with their versions/effective dates** (§7). |
| `fires[]` | Index of fires: id, label, and per-fire run index. |
| `runs[]` | Per run: engine, timestamp, output inventory, and **`derivedFrom`** lineage (§9.3). |
| `provenance[]` | Index mapping each **canonical artifact → its original(s)** (§8). |
| `contents` | Declaration of which OPTIONAL parts are present (§10). |
| `checksums` | **[TBD-v1.0]** per-artifact integrity hashes. |

Schema formalisation (JSON Schema) is **[TBD-v1.0]**.

## 7. Coordinate reference & units
...
- Units for each canonical quantity (e.g. weather variables) are **fixed by the canonical schema** (§9), not free-form.
...
## 8. Originals & provenance (losslessness)
...
- **Weather** is carried in a **canonical, versioned, engine-neutral schema** — *not* an engine-specific CSV. The schema version is declared in the manifest. Import coerces any source into this schema **and keeps the original** (§8). The concrete weather schema is **[TBD-v1.0]**.
- Ignition and perimeter canonical schemas: **[TBD-v1.0]**.

### 9.3 Runs, models & lineage
...
## 10. Required vs optional (essentials)
...
- A **conformant writer** produces files honouring §4–§10.
